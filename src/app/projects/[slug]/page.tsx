      ...
      <div className="space-y-8">
          ...
          <>
              ...
              {project.techStack && (
                <div className="flex flex-wrap gap-2">
                  {project.techStack.map((tech) => (
                    <span
                      key={tech}
                      className="text-sm px-3 py-1 bg-gray-100 text-gray-700 rounded border border-gray-200"
                    >
                      {tech}
                    </span>
                  ))}
              ...
              {project.repoUrl && (
                <div className="mt-6">
                  <a
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 bg-gray-900 text-white rounded border border-gray-900 hover:bg-gray-800 transition-colors"
                  >
            ...
            {project.tags && project.tags.length > 0 && (
              <div className="pt-8 border-t border-gray-200">
                <h3 className="text-sm font-semibold text-gray-900 mb-3">Project Tags</h3>
                <div className="flex flex-wrap gap-2">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-3 py-1 text-sm bg-gray-50 text-gray-700 rounded border border-gray-200"
                    >
                      {tag}
                    </span>
                  ))}
        ...
        {project.category === "PC Building" && (
          <>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-8">
                ...
                <div className="space-y-3">
                  {project.components.map((component, idx) => (
                    <div
                      key={idx}
                      className="flex items-start gap-3 text-gray-700 pb-3 border-b border-gray-200 last:border-0"
                    >
            ...
            {/* Build Photos */}
            <div className="space-y-6">
              <h2 className="text-2xl font-semibold text-gray-900">Build Photos</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div
                    key={i}
                    className="aspect-square bg-gray-100 border border-gray-200 rounded-lg flex items-center justify-center"
                  >
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {Array.from({ length: project.imageCount || 12 }).map((_, i) => (
                  <div
                    key={i}
                    className="aspect-square bg-gray-100 border border-gray-200 rounded-lg flex items-center justify-center hover:bg-gray-200 transition-colors cursor-pointer"
                  >
